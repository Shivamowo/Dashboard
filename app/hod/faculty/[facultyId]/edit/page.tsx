...
import {
  ...
  submitHodFacultyResearch,
  submitHodFacultyTarget,
} from "@/lib/actions";
...
export default async function HodFacultyEditPage({
  params,
  searchParams,
}: {
    ...
    return (
      <div>
        ...
        <EmptyState
          ...
          message={`As head of ${dept.name} you can edit records for your own faculty only.`}
        />
      </div>
    );
  }
  ...
  const mine = requestsSubmittedByUser(user.id).filter((c) => c.targetEntity === "Faculty" && c.targetId === f.id);
  ...
  const submitProfile = submitHodFacultyProfile.bind(null, f.id);
  const submitResearch = submitHodFacultyResearch.bind(null, f.id);
  const submitTarget = submitHodFacultyTarget.bind(null, f.id);
  const submitProjects = submitHodFacultyProjects.bind(null, f.id);
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <Section title="Identity & teaching" description="Every field on the appointment record.">
