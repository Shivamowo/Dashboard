...
import { facultyById, projectsOf, researchOf, targetOf } from "@/data";
import {
  adminUpdateFacultyProfile,
  adminUpdateFacultyProjects,
  adminUpdateFacultyResearch,
  adminUpdateFacultyTarget,
} from "@/lib/actions";
...
export default async function AdminFacultyEditPage({ params }: { params: Promise<{ facultyId: string }> }) {
  ...
  const submitProfile = adminUpdateFacultyProfile.bind(null, f.id);
  const submitResearch = adminUpdateFacultyResearch.bind(null, f.id);
  const submitTarget = adminUpdateFacultyTarget.bind(null, f.id);
  const submitProjects = adminUpdateFacultyProjects.bind(null, f.id);
    ...
    <div>
      <PageHeading
        crumbs={[
          ...
          { label: f.name, href: `/admin/faculty/${f.id}` },
          { label: "Edit" },
        ]}
        title={`Edit ${f.name}`}
      ...
      <div className="space-y-6">
        <Section title="Identity & teaching">
          <form action={submitProfile} className="space-y-5">
            <FormGrid cols={3}>
              ...
              <TextField name="dateOfJoining" label="Date of Joining" defaultValue={f.dateOfJoining} required />
